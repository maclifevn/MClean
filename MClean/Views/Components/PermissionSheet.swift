...
/// and offers escape hatches for the "MClean isn't in the list" case.
struct PermissionSheet: View {
    ...
    var body: some View {
        ...
        .animation(reduceMotion ? nil : .spring(response: 0.4, dampingFraction: 0.8),
                   value: coordinator.needsRelaunch)
        .onAppear {
            if reduceMotion {
                appeared = true
                return
    ...
    private var headerCaption: LocalizedStringKey {
        if coordinator.hasFullDiskAccess {
            return "Access granted. Retrying…"
        }
        if coordinator.needsRelaunch {
            return "Access granted — relaunch to apply"
        }
        return "1-tap setup. We'll detect the change automatically."
    }
    ...
    private var body_: some View {
        ...
        } else if coordinator.needsRelaunch {
            relaunchBody
                .transition(
                    reduceMotion
                        ? .opacity
    ...
    /// The toggle is on in System Settings, but macOS applies FDA only to
    /// processes started after the grant — the running instance must restart.
    private var relaunchBody: some View {
        VStack(spacing: 10) {
            SuccessMedal(size: 72)
            Text("Access granted")
                .font(.system(size: 15, weight: .bold))
            Text("macOS applies Full Disk Access when MClean starts again. Relaunch to finish — takes a second.")
    ...
    private var requestBody: some View {
        VStack(alignment: .leading, spacing: 18) {
            // Two paths the user can choose between: drag the MClean bundle
            ...
            HStack(alignment: .center, spacing: 14) {
                ...
                VStack(spacing: 8) {
                    Button {
                        ...
                        Label("Open Settings & reveal MClean", systemImage: "gear")
            ...
            // Step-by-step strip
            HStack(spacing: 0) {
                stepCell(number: 1, title: "Turn on MClean",
            ...
            HStack(spacing: 8) {
                ...
                Text("Watching for permission change…")
                    ...
                    .foregroundStyle(.secondary)
                Spacer()
                Button(showAdvanced ? "Hide help" : "MClean not in the list?") {
    ...
    private var advancedPanel: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("Try this if MClean doesn't appear:")
            ...
            HStack(spacing: 8) {
                advancedButton(
                    ...
                    title: "Reveal app",
                    subtitle: "Drag MClean.app into the list"
                ) {
    ...
    private var footer: some View {
        HStack(spacing: 12) {
            ...
            if coordinator.needsRelaunch && !coordinator.hasFullDiskAccess {
                Button {
                    coordinator.relaunch()
                } label: {
                    Label("Relaunch MClean", systemImage: "arrow.clockwise")
                }
                .buttonStyle(.borderedProminent)
                .controlSize(.regular)
                .keyboardShortcut(.defaultAction)
            } else if !coordinator.hasFullDiskAccess {
