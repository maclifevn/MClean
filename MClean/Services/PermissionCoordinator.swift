...
/// - Auto-opens the Settings pane and reveals MClean.app in Finder so users can
...
final class PermissionCoordinator: ObservableObject {
    ...
    /// True when the FDA toggle is on in System Settings but this process
    /// launched before the grant — macOS only applies FDA to new processes,
    /// so the app must relaunch for it to take effect. Detected by a
    /// fresh-child-process probe succeeding while the in-process probe fails.
    ...
    /// Serializes the fresh-process probe so poll ticks never stack child
    /// processes if one probe runs long.
    private var freshProbeInFlight = false
    ...
    /// Open System Settings AND reveal MClean.app in Finder so the user can
    ...
    /// Reset MClean's TCC entry and re-prime registration. Use when the user
    /// reports MClean doesn't appear in the FDA list (typical after Homebrew
        ...
        pollTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
            Task { @MainActor [weak self] in
                ...
                if granted && !self.hasFullDiskAccess {
                    ...
                    self.hasFullDiskAccess = granted
                    if !granted { self.checkForPendingRelaunch() }
                }
            }
        }
    ...
    /// In-process probe says denied — ask a fresh child process. If the child
    /// can read TCC-protected files, the user HAS granted access and only a
    /// relaunch is missing; flip `needsRelaunch` so the sheet swaps its
    /// "watching…" state for a Relaunch button. Without this the sheet polls
    /// forever: macOS never applies an FDA grant to an already-running process.
    private func checkForPendingRelaunch() {
        guard !needsRelaunch, !freshProbeInFlight else { return }
        freshProbeInFlight = true
        Task.detached(priority: .userInitiated) {
            let grantedInFreshProcess =
                FullDiskAccessManager.shared.hasFullDiskAccessInFreshProcess()
            await MainActor.run { [weak self] in
                guard let self else { return }
                self.freshProbeInFlight = false
                if grantedInFreshProcess, !self.hasFullDiskAccess {
    ...
    /// Restart the app so a granted-but-not-applied FDA toggle takes effect.
    func relaunch() {
        FullDiskAccessManager.shared.relaunchApp()
    }
